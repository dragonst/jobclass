...
<div id="companyFields">
	...
	<div class="form-group required <?php echo (isset($errors) and $errors->has('employee.firstname')) ? 'has-error' : ''; ?>">
		<label class="{{ $classLeftCol }} control-label" for="employee.firstname">{{ t('First Name') }} <sup>*</sup></label>
		<div class="{{ $classRightCol }}">
			<input name="employee[firstname]"
				   placeholder="{{ t('First Name') }}"
				   class="form-control input-md"
				   type="text"
				   value="{{ old('employee.firstname', (isset($employee->firstname) ? $employee->firstname : '')) }}">
		</div>
	</div>

	<!-- logo -->
  <div class="form-group required <?php echo (isset($errors) and $errors->has('employee.lastname')) ? 'has-error' : ''; ?>">
		<label class="{{ $classLeftCol }} control-label" for="employee.lastname">{{ t('Last Name') }} <sup>*</sup></label>
		<div class="{{ $classRightCol }}">
			<input name="employee[lastname]"
				   placeholder="{{ t('Last Name') }}"
				   class="form-control input-md"
				   type="text"
				   value="{{ old('employee.lastname', (isset($employee->lastname) ? $employee->lastname : '')) }}">
		</div>
	</div>

  <div class="form-group required <?php echo (isset($errors) and $errors->has('employee.position')) ? 'has-error' : ''; ?>">
    <label class="{{ $classLeftCol }} control-label" for="employee.position">{{ t('Position') }} <sup>*</sup></label>
    <div class="{{ $classRightCol }}">
      <input name="employee[position]"
           placeholder="{{ t('Position') }}"
           class="form-control input-md"
           type="text"
           value="{{ old('employee.position', (isset($employee->position) ? $employee->position : '')) }}">
    </div>
  </div>



  <div class="form-group required <?php echo (isset($errors) and $errors->has('employee.rating_count')) ? 'has-error' : ''; ?>">
    <label class="{{ $classLeftCol }} control-label" for="employee.rating_count">{{ t('Rating') }} <sup>*</sup></label>
    <div class="{{ $classRightCol }}">

            <div class="starrr" id="star1"></div>
						<div>&nbsp;

        <span class="choice" style="disply:none;"></span>
				<input type="hidden" id="rating_count" value="{{ old('employee.position', (isset($employee->position) ? $employee->position : '')) }}" name="employee[rating_count]">
	...
	<!-- description -->
	<div class="form-group required <?php echo (isset($errors) and $errors->has('employee.comment')) ? 'has-error' : ''; ?>">
		<label class="{{ $classLeftCol }} control-label" for="employee.comment">{{ t('Leave a comment') }} <sup>*</sup></label>
		<div class="{{ $classRightCol }}">
			<textarea class="form-control" name="employee[comment]" rows="10">{{ old('employee.comment', (isset($employee->comment) ? $employee->comment : '')) }}</textarea>
			<p class="help-block">{{ t('Leave a comment') }} - ({{ t(':number characters maximum', ['number' => 1000]) }})</p>
		</div>
	</div>
...
@section('after_scripts')
	...
	<script>
		...
		$('#logo').fileinput(
		{
			...
			initialPreview: [
				'<img src="{{ resize($company->logo, 'medium') }}" class="file-preview-image">',
			],

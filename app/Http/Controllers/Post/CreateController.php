...
class CreateController extends FrontController
{
	...
	public function postForm($tmpToken = null, PostRequest $request)
	{
		...
		$postInfo = [
			...
			'tags'                => $request->filled('tags') ? strtolower($request->input('tags')) : null,
			...
			'lat'                 => $city->latitude,
			'lon'                 => $city->longitude,
			'application_url'     => $request->filled('application_url') ? strtolower($request->input('application_url')) : null,
			'ip_addr'             => Ip::get(),

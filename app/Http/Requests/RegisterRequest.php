...
class RegisterRequest extends Request
{
    ...
    public function rules()
    {
        ...
        if ($this->filled('phone')) {
            ...
            $rules['phone'] = 'phone:' . $countryCode . ',mobile|unique:users,phone|' . $rules['phone'];
        }
		...
		if (config('larapen.core.register.showResumeFields')) {
			if ($this->input('user_type') == 3) {
				$rules['resume.filename'] = 'required|mimes:' . getUploadFileTypes('file') . '|max:' . (int)config('settings.upload_max_file_size', 1000);
			}
		}

...
class LaravelLocalization
{
    ...
    public function __construct()
    {
        ...
        self::$cacheExpiration = (int)config('settings.app_cache_expiration', 60);
    ...
    public function getLocalizedURL($locale = null, $url = null, $attributes = [], $params = [])
    {
        ...
        if (empty($url)) {
            ...
            // Search : Index
            if (str_contains(Route::currentRouteAction(), 'Search\SearchController@index')) {
                // Build URL
                if (isset($params['countryCode'])) {
                    $r_url = trans('routes.v-search', ['countryCode' => $params['countryCode']], $locale);
                } else {
                    $r_url = trans('routes.v-search', [], $locale);
                }
                $url = app('url')->to($preUri . $r_url) . $queryString;

                return $url;
            } // Search : Category
            else if (str_contains(Route::currentRouteAction(), 'Search\CategoryController@index')) {
                // Get category or sub-category translation
                if (isset($params['catSlug'])) {
                    $cat = self::getCategoryBySlug($params['catSlug'], $locale);
                    if (!is_null($params['subCatSlug']) and !empty($cat)) {
                        $subCat = self::getSubCategoryBySlug($cat->tid, $params['subCatSlug'], $locale);
                    }

                    // Build URL
                    $r_url = '';
                    if (isset($cat) and isset($subCat) and !empty($cat) and !empty($subCat)) {
                        ...
                        if (isset($cat) and !empty($cat)) {
                            $r_url = trans('routes.v-search-cat', ['countryCode' => $params['countryCode'], 'catSlug' => $cat->slug], $locale);
                        }
                    }
                    $url = app('url')->to($preUri . $r_url) . $queryString;
                ...
                if (isset($params['cityName'])) {
                    $r_url = trans('routes.v-search-city', ['city' => $params['cityName'], 'id' => $params['cityId']], $locale);
                    $url = app('url')->to($preUri . $r_url) . $queryString;
                ...
                if (isset($params['pageSlug'])) {
                    $page = self::getPageBySlug($params['pageSlug'], $locale);

                    if (!empty($page)) {
                        $r_url = trans('routes.v-page', ['slug' => $page->slug], $locale);
                        $url = app('url')->to($preUri . $r_url) . $queryString;
